...
LAR uses a **single package kind**. There is no required package `type` or `role` field. Distribution uses one **package source** abstraction; see [architecture.md](architecture.md).
